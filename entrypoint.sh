...
if [ ! -f "/work/.cloudflare/cloudflare.ini" ]; then
	echo "...[x]"
	echo -n "Make cloudflare.ini"
	
	# 파일 생성
	mkdir -p /work/.cloudflare
	echo "dns_cloudflare_api_token = $CLOUDFLARE_API_TOKEN" >/work/.cloudflare/cloudflare.ini
	chmod 600 /work/.cloudflare/cloudflare.ini

	# 성공 여부 확인
	if [ -f "/work/.cloudflare/cloudflare.ini" ]; then
		echo "...[v]"
	else
		echo "ERROR: cloudflare.ini 생성에 실패했습니다. /work/.cloudflare/cloudflare.ini"
		exit 1
	fi
else
	echo "...[v]"
...
if [ ! -d "/etc/letsencrypt/live/$DOMAIN" ]; then
	LOG_FILE="/work/logs/certbot_issue.log"
	...
	certbot certonly --dns-cloudflare \
		...
		--dns-cloudflare-credentials /work/.cloudflare/cloudflare.ini \
		--non-interactive \
		--agree-tos \
		-m "$EMAIL" \
		-d "*.$DOMAIN" \
